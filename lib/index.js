 ...
 */
exports.authenticate = ({strategy, req, res, options = {}}) => {
  ...
  if(strategy === 'session') {
    ...
    if(origin === null || req.headers.host === origin ||
      ...
      if(options.allowUrlEncoded || !(
        ...
        // code below with
        // `app.use(exports.createMiddleware({strategy: 'session'}))`
 ...
 */
exports.authenticateAll = async ({req, res, options = {}}) => {
 ...
 */
exports.createMiddleware = ({strategy, options = {}}) => {
  return async (req, res, next) => {
    try {
      const {user} = await exports.authenticate({strategy, req, res, options});
 ...
 */
exports.optionallyAuthenticated = async (req, res, next) => {
  try {
    const {user} = await exports.authenticateAll({req, res});
    // if authentication found, set req.user
    if(user) {
      req.user = user;
    }
...
exports.ensureAuthenticated = (req, res, next) => {
  exports.optionallyAuthenticated(req, res, err => {
    if(err) {
      return next(err);
    }
    // authenticated
 ...
 *                any.
 */
exports.createAuthenticator = options => {
  options = options || {};
  return async (req, res, next) => {
    try {
      const {user} = await exports.authenticateAll({req, res, options});
      // if authentication found, set req.user
      if(user) {
        req.user = user;
      }
...
bedrock.events.on('bedrock-express.configure.router', function configure(app) {
  ...
  passport.deserializeUser(callbackify(exports._deserializeUser));
...
exports._deserializeUser = async function(data) {
  ...
  if(typeof data.accountId === 'string') {
    try {
      const record = await brAccount.get({id: data.accountId});
      ...
      throw new BedrockError(
        e.message, e.name, {public: false, id: data.accountId}, e);
    }
  }
